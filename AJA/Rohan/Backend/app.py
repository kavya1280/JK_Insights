        ...
        if 'concurFile' in request.files:
            ...
            if os.path.exists(concur_path):
                try:
                    ...
                    # Employee ID
                    emp_col = next((c for c in df_c.columns
                                    if 'employee id' in c.lower() or 'emp id' in c.lower()), None)
                    if emp_col:
                        kpis['unique_employees'] = int(df_c[emp_col].nunique())

                    # Amount Approved
                    amt_col = next((c for c in df_c.columns if 'amount' in c.lower()), None)
                    if amt_col:
                        total_amt = float(pd.to_numeric(df_c[amt_col], errors='coerce').fillna(0).sum())
                        kpis['total_amount']  = round(total_amt, 2)
                        kpis['average_claim'] = round(
                            total_amt / kpis['total_transactions'], 2
                        ) if kpis['total_transactions'] > 0 else 0.0

                    # Report ID
                    rep_col = next((c for c in df_c.columns if 'report' in c.lower() and 'id' in c.lower()), None)
                    if not rep_col:
                        rep_col = next((c for c in df_c.columns if 'report' in c.lower()), None)
                    if rep_col:
                        kpis['unique_reports'] = int(df_c[rep_col].nunique())

                    print(f"[KPI] Concur → rows={kpis['total_transactions']}, "
                          f"emp={kpis['unique_employees']}, reports={kpis['unique_reports']}, "
                          f"amount={kpis['total_amount']}, avg={kpis['average_claim']}")
                except Exception as e:
                    print(f"[KPI] Concur extraction error: {e}")
        ...
        if 'empMasterFile' in request.files:
            ...
            if os.path.exists(emp_path):
                try:
                    ...
                    # Employee ID column — try known names then fallback to column[1]
                    id_col = next((c for c in df_e.columns
                                   if 'employee id' in c.lower() or 'emp id' in c.lower()
                                   or 'empno' in c.lower()), None)
                    if not id_col and len(df_e.columns) > 1:
                        id_col = df_e.columns[1]

                    if id_col:
                        df_e['_emp_id'] = (df_e[id_col].astype(str).str.strip()
                                           .str.replace(r'\.0$', '', regex=True))
                        kpis['master_unique_employees'] = int(df_e['_emp_id'].nunique())

                        # Status
                        status_col = next((c for c in df_e.columns
                                           if 'status' in c.lower()), None)
                        if status_col:
                            df_e['_status'] = df_e[status_col].astype(str).str.strip().str.upper()
                            sep_ids = df_e[df_e['_status'] != 'ACTIVE']['_emp_id'].unique()
                            kpis['master_separated_employees'] = len(sep_ids)
                            kpis['master_active_employees'] = (kpis['master_unique_employees']
                                                               - kpis['master_separated_employees'])
                        else:
                            # Fallback: check for separation date column
                            sep_col = next((c for c in df_e.columns
                                            if 'sep' in c.lower() or 'termination' in c.lower()
                                            or 'exit' in c.lower()), None)
                            if sep_col:
                                kpis['master_separated_employees'] = int(df_e[sep_col].notna().sum())
                                kpis['master_active_employees'] = (kpis['master_unique_employees']
                                                                   - kpis['master_separated_employees'])
                            else:
                                kpis['master_active_employees'] = kpis['master_unique_employees']
                    ...
                    # Company Code
                    company_col = next((c for c in df_e.columns
                                        if 'company code' in c.lower() or 'company' in c.lower()), None)
                    if company_col:
                        kpis['master_company_codes'] = int(df_e[company_col].nunique())

                    print(f"[KPI] EmpMaster → total={kpis['master_unique_employees']}, "
                          f"active={kpis['master_active_employees']}, "
                          f"sep={kpis['master_separated_employees']}, "
                          f"companies={kpis['master_company_codes']}")
                except Exception as e:
                    print(f"[KPI] Employee Master extraction error: {e}")

        users = load_users()
        user  = next((u for u in users if u["username"] == username), None)
        role  = user["role"] if user else "uploader"
        log_activity(username, role, "FILE_UPLOAD",
                     f"Uploaded: {', '.join(request.files.keys())}")
