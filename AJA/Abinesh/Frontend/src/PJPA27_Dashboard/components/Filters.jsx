...
const FilterIcons = {
    id: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2" /><line x1="2" y1="10" x2="22" y2="10" /></svg>),
    status: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><polyline points="22 4 12 14.01 9 11.01" /></svg>),
    risk: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z" /></svg>),
    policy: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>),
    designation: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z" /><line x1="7" y1="7" x2="7.01" y2="7" /></svg>)
};
...
const handleScreenshot = async () => {
    try {
        ...
        link.download = `pjpa27_dashboard_${new Date().toISOString().slice(0, 10)}.png`;
...
const Filters = ({ options, filters, onFilterChange, onReset }) => {
    const uniqueValues = {
        ...
        risk_category: options?.risk_categories || [],
    ...
    const filterConfigs = [
        { name: 'employee_id', label: 'Employee ID', icon: FilterIcons.id, placeholder: 'All' },
        { name: 'payment_status', label: 'Payment Status', icon: FilterIcons.status, placeholder: 'All' },
        { name: 'risk_category', label: 'Risk Category', icon: FilterIcons.risk, placeholder: 'All' },
        { name: 'policy', label: 'Policy', icon: FilterIcons.policy, placeholder: 'All' },
        { name: 'designation', label: 'Designation Name', icon: FilterIcons.designation, placeholder: 'All' }
    ];
        ...
        <div className="filters-section">
            <div className="filters-header">
                ...
                <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                    <button className="screenshot-btn" onClick={handleScreenshot} title="Take Screenshot (PNG)" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '36px', height: '36px', borderRadius: '8px', border: '1px solid #e0e0e0', background: 'white', cursor: 'pointer', transition: 'all 0.2s', padding: 0 }}>{ScreenshotIcon}</button>
