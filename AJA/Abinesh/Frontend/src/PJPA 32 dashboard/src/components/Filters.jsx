...
const Filters = ({ options, filters, onFilterChange, onReset }) => {
  const uniqueValues = {
    employeeId: (options?.employee_ids || []).map(String),
    employee: options?.employees || [],
    reportId: (options?.report_ids || []).map(String),
    expenseType: options?.expense_types || [],
  ...
  const filterConfigs = [
    ...
    { name: 'reportId', label: 'Report ID', icon: FilterIcons.reportId, placeholder: 'Select Report' },
    { name: 'expenseType', label: 'Expense Type', icon: FilterIcons.expenseType, placeholder: 'Select Type' },
