...
const PREFERRED = [
    ...
    { key: 'Notice Period Bucket', label: 'Notice Bucket' },
    { key: 'Joining_Date', label: 'Joining Date' },
    { key: 'Resignation Date', label: 'Resignation Date' },
    { key: 'Flag (Reason)', label: 'Reason' }
];
...
const DataTable = ({ filters, onFilterChange, debouncedSearch }) => {
    ...
    useEffect(() => { setCurrentPage(1); }, [filters.employee_id, filters.department, filters.notice_bucket, filters.cost_center, filters.employee_status, pageSize, debouncedSearch]);
    ...
    useEffect(() => {
        ...
        const doFetch = async () => {
            ...
            try {
                ...
                const res = await fetch(`http://localhost:8000/api/pjpa39/table?${qs}`);
    ...
    }, [filters.employee_id, filters.department, filters.notice_bucket, filters.cost_center, filters.employee_status, currentPage, pageSize, debouncedSearch]);

    const handleDownloadCSV = useCallback(async () => {
        try {
            setCsvLoading(true);
            const qs = buildQuery(filters, { page: 1, page_size: 100000, search: debouncedSearch || undefined });
            const res = await fetch(`http://localhost:8000/api/pjpa39/table/csv?${qs}`);
    ...
    }, [filters.employee_id, filters.department, filters.notice_bucket, filters.cost_center, filters.employee_status, debouncedSearch]);
