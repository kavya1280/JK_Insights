...
const INSIGHT_OPTIONS = [
  ...
  { id: "PJPA32_HOL", label: "PJPA32 - Holiday Travel", req: ["lineItemFile"] },
...
const FILE_TYPES = [
  { key: "concurFile", label: "Concur Header Data", sub: "Required for Header Analysis" },
  { key: "lineItemFile", label: "Line Item Expenses", sub: "Required for Transactional Analysis" },
  { key: "empMasterFile", label: "Employee Master", sub: "Required for Joiner/Tenure Analysis" },
  { key: "leftEmpFile", label: "Left Employees", sub: "Required for Notice Period Analysis" },
];
...
const Uploader = ({ logo, handleLogout }) => {
  ...
  const handleUploadSubmit = async () => {
    ...
    if (fileCount === 0) {
      alert("No files selected for upload. Please select at least one file.");
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorMsg = typeof errData.detail === 'object'
          ...
          : (errData.detail || errData.message || "Unknown error");
  ...
  const startAnalysis = async () => {
    ...
    // 1. Force add all selected insights to toRun (Backend has data already)
    ...
    if (toRun.length > 0) {
      try {
        ...
        const fetchPromises = toRun.map(id =>
          fetch(`http://localhost:8000/api/insight/${id}/data`).then(res => ({ id, res }))
        );
  ...
  const handleInlineReupload = async (e, reportItem, fileKey) => {
    ...
    try {
      setHistory(prev => prev.map(item => item.id === reportItem.id ? { ...item, reason: "Uploading & Processing..." } : item));
      ...
      const dataRes = await fetch(`http://localhost:8000/api/insight/${reportItem.moduleId}/data`);
    ...
    <div className="up-split-layout animate-in" style={{ display: 'flex', maxWidth: '1200px', width: '100%', minHeight: '600px', background: 'white', borderRadius: '24px', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.1)' }}>
      <section style={{ flex: 1.2, padding: '40px' }}>
        ...
        <div style={{ marginTop: '40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f1f5f9', paddingTop: '20px' }}>
          <a href="/src/assets/Sampledata.csv" style={{ color: '#05192d', fontWeight: 'bold', fontSize: '14px', textDecoration: 'none', borderBottom: '2px solid #00df81' }}>View Sample.csv</a>
          <button onClick={handleUploadSubmit} disabled={isUploading || !files.concurFile} style={{ padding: '14px 30px', background: '#05192d', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold', cursor: isUploading ? 'not-allowed' : 'pointer', boxShadow: '0 4px 14px rgba(5,25,45,0.2)' }}>
  ...
  const renderKPIView = () => (
    <div className="animate-in" style={{ width: '100%', maxWidth: '1200px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        ...
        <button onClick={() => setView("upload")} style={{ padding: '8px 15px', borderRadius: '8px', border: '1px solid #cbd5e1', background: 'white', cursor: 'pointer' }}>← Change Files</button>
      </div>
      ...
      {activeAnalysisResults.map((insight) => (
        <div key={insight.id}>
          {currentViewMode === "table" ? (
            ...
            insight.moduleId.startsWith("PJPA32") ? (
              <PJPA32_Dashboard onBack={handleReportToggle} data={insight.data} moduleId={insight.moduleId} onBackToTable={() => setCurrentViewMode("table")} />
            ) : insight.moduleId === "PJPA29" ? (
              <PJPA29_Dashboard onBack={handleReportToggle} data={insight.data} />
            ) : insight.moduleId === "PJPA30" ? (
              <PJPA30_Dashboard onBack={handleReportToggle} data={insight.data} />
            ) : insight.moduleId === "PJPA31" ? (
              <PJPA31_Dashboard onBack={handleReportToggle} data={insight.data} />
    ...
    <div className="animate-in" style={{ width: '100%', maxWidth: '1200px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          <button onClick={() => setView("kpi_overview")} style={{ background: '#f1f5f9', border: 'none', padding: '8px 15px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}>← Control Selection</button>
                    ...
                    {item.status === 'Success' ? (
                      <div style={{ display: 'flex', gap: '8px' }}>
                        ...
                        <button
                          ...
                          style={{ padding: '8px 16px', background: '#00df81', color: '#05192d', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px' }}>
                          Dashboard View
                        </button>
                      </div>
                    ) : (
