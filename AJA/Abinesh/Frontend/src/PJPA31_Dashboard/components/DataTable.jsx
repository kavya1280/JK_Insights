...
const COLUMNS = [
    ...
    { key: 'computedDate', label: 'Date', isDate: true },
    { key: 'computedSplittingStatus', label: 'Splitting Status' },
    { key: 'Department', label: 'Department' },
    { key: 'Amount Approved', label: 'Total Amount', isAmount: true },
    { key: 'computedApprovedStatus', label: 'Approved Status' }
];
...
const DataTable = ({ data }) => {
    ...
    const enrichedData = useMemo(() => {
        ...
        // We try to make sure 'Date', 'Splitting Status', 'Approved Status' exist or map somewhat to raw columns
        return data.map(item => ({
            ...item,
            computedDate: item['Submit Date'] || item['Date'] || item['Transaction Date'],
            computedSplittingStatus: item['Splitting Status'] || item['Risk Category'] || 'Unknown',
            computedApprovedStatus: item['Approval Status'] || item['Approved Status'] || 'Unknown'
        }));
    }, [data]);
    ...
    const handleDownloadCSV = () => {
        ...
        const csvLines = [
            ...
            ...filteredData.map(row => COLUMNS.map(c => escape(formatVal(c, row[c.key]))).join(','))
        ];
        const blob = new Blob([csvLines.join('\r\n')], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = 'pjpa31_data.csv'; a.click();
        ...
        <div className="data-table-card">
            <div className="table-header-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <h3 className="chart-title" style={{ margin: 0 }}>Splitting Analysis Data</h3>
