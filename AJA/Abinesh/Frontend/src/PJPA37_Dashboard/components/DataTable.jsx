...
const PREFERRED = [
    ...
    { key: 'Amount Approved', label: 'Approved Amount', isAmount: true },
...
const formatVal = (col, val) => {
    ...
    if (col.key === 'Is_Anomaly') return val === 1 || val === "1" ? "Yes" : "No";
    return String(val);
};
...
const DataTable = ({ filters, onFilterChange, debouncedSearch }) => {
    ...
    useEffect(() => { setCurrentPage(1); }, [filters.employee_id, filters.policy, filters.cluster_id, filters.is_anomaly, pageSize, debouncedSearch]);
    ...
    useEffect(() => {
        ...
        const doFetch = async () => {
            ...
            try {
                ...
                const res = await fetch(`http://localhost:8000/api/pjpa37/table?${qs}`);
    ...
    }, [filters.employee_id, filters.policy, filters.cluster_id, filters.is_anomaly, currentPage, pageSize, debouncedSearch]);

    const handleDownloadCSV = useCallback(async () => {
        try {
            setCsvLoading(true);
            const qs = buildQuery(filters, { page: 1, page_size: 100000, search: debouncedSearch || undefined });
            const res = await fetch(`http://localhost:8000/api/pjpa37/table/csv?${qs}`);
            const text = await res.text();

            const blob = new Blob([text], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = 'anomaly_detection_dashboard.csv'; a.click();
        ...
        <div className="data-table-card">
            <div className="table-header-row" style={{ alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <h3 className="chart-title" style={{ margin: 0 }}>Anomaly Tracking Data</h3>
                                    ...
                                    <tr key={idx} className={idx % 2 === 0 ? 'row-even' : 'row-odd'}>
                                        ...
                                        {columns.map(col => (
                                            <td key={col.key} className={col.isAmount ? 'text-right td-amount' : ''}>
                                                {col.key === 'Is_Anomaly' && (row[col.key] === 1 || row[col.key] === "1") ? <span style={{ color: '#EF5350', fontWeight: 'bold' }}>Yes</span> : formatVal(col, row[col.key])}
                                            </td>
                                        ))}
                                    </tr>
