...
const PREFERRED = [
    ...
    { key: 'Approved Amount', label: 'Approved Amount', isAmount: true },
    { key: 'Flag', label: 'Flag' },
    { key: 'Mode_Count', label: 'Mode Count' },
    { key: 'Usage_Pct', label: 'Usage Pct (%)' },
...
const formatVal = (col, val) => {
    ...
    if (col.key === 'Flag' && (val === 'RARE' || val === 'rare')) {
        return <span style={{ color: '#EF5350', fontWeight: 'bold' }}>{val}</span>;
    }
    return String(val);
};
...
const DataTable = ({ filters, onFilterChange, debouncedSearch }) => {
    ...
    useEffect(() => { setCurrentPage(1); }, [filters.employee, filters.expense_type, filters.flag, pageSize, debouncedSearch]);
    ...
    useEffect(() => {
        ...
        const doFetch = async () => {
            ...
            try {
                ...
                const res = await fetch(`http://localhost:8000/api/pjpa38/table?${qs}`);
    ...
    }, [filters.employee, filters.expense_type, filters.flag, currentPage, pageSize, debouncedSearch]);

    const handleDownloadCSV = useCallback(async () => {
        try {
            setCsvLoading(true);
            const qs = buildQuery(filters, { page: 1, page_size: 100000, search: debouncedSearch || undefined });
            const res = await fetch(`http://localhost:8000/api/pjpa38/table/csv?${qs}`);
            const text = await res.text();

            const blob = new Blob([text], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = 'odd_travels_dashboard.csv'; a.click();
        ...
        <div className="data-table-card">
            <div className="table-header-row" style={{ alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <h3 className="chart-title" style={{ margin: 0 }}>Odd Travels Data</h3>
