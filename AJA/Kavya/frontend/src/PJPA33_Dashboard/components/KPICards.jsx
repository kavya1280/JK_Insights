...
const KPICards = ({ kpis }) => {
    ...
    const stats = {
        ...
        distinctEmployeeName: kpis?.distinct_employee_name || 0,
        approvalStatusCount: kpis?.approval_status_count || 0,
        paymentStatusCount: kpis?.payment_status_count || 0,
        totalReportAmount: kpis?.total_report_amount || 0,
    ...
    const cards = [
        { label: 'Employee ID (Count)', value: stats.distinctEmployeeId, type: 'number', icon: Icons.employeeId, color: '#6FAE2C' },
        { label: 'Employee Name (Count)', value: stats.distinctEmployeeName, type: 'number', icon: Icons.employee, color: '#0B4F94' },
        { label: 'Approval Status (Count)', value: stats.approvalStatusCount, type: 'number', icon: Icons.status, color: '#6FAE2C' },
        { label: 'Payment Status (Count)', value: stats.paymentStatusCount, type: 'number', icon: Icons.status, color: '#0B4F94' },
        { label: 'Report Total', value: stats.totalReportAmount, type: 'currency', icon: Icons.currency, color: '#6FAE2C' },
        { label: 'Amount Due Employee', value: stats.amountDueEmployee, type: 'currency', icon: Icons.currency, color: '#0B4F94' },
        { label: 'Amount Approved', value: stats.amountApproved, type: 'currency', icon: Icons.currency, color: '#6FAE2C' }
    ];
