...
const PREFERRED = [
    ...
    { key: 'Report Number', label: 'Report Number' },
    ...
    { key: 'Amount Due Employee', label: 'Amount Due Employee', isAmount: true },
    { key: 'Report Start Date', label: 'Report Start Date' },
    { key: 'Report End Date', label: 'Report End Date' },
...
const DataTable = ({ filters, onFilterChange, debouncedSearch }) => {
    ...
    useEffect(() => { setCurrentPage(1); }, [filters.employee_id, filters.employee, filters.report_id, filters.approval_status, filters.payment_status, pageSize, debouncedSearch]);
    ...
    useEffect(() => {
        ...
        const doFetch = async () => {
            ...
            try {
                ...
                const res = await fetch(`http://localhost:5000/api/pjpa33/table?${qs}`);
    ...
    }, [filters.employee_id, filters.employee, filters.report_id, filters.approval_status, filters.payment_status, currentPage, pageSize, debouncedSearch]);
    ...
    const handleDownloadCSV = useCallback(async () => {
        try {
            ...
            const res = await fetch(`http://localhost:5000/api/pjpa33/table/csv?${qs}`);
            const text = await res.text();

            const blob = new Blob([text], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = 'bulk_booking_reimbursements.csv'; a.click();
    ...
    }, [filters.employee_id, filters.employee, filters.report_id, filters.approval_status, filters.payment_status, debouncedSearch]);
        ...
        <div className="data-table-card">
            <div className="table-header-row" style={{ alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <h3 className="chart-title" style={{ margin: 0 }}>Bulk Booking Data</h3>
