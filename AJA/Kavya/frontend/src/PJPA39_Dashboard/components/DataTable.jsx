...
const PREFERRED = [
    { key: 'Position Code', label: 'Position Code' },
    { key: 'Personnel Number', label: 'Personnel Number' },
    { key: 'Employee ID', label: 'Employee ID' },
    { key: 'Employee Status', label: 'Employee Status' },
    { key: 'Supplier', label: 'Supplier' },
    { key: 'Position Code Name', label: 'Position Code Name' },
    { key: 'Full Name', label: 'Full Name' },
    { key: 'Title', label: 'Title' },
    { key: 'Employee Email Id', label: 'Employee Email Id' },
    { key: 'Phone Number', label: 'Phone Number' },
    { key: 'Employee Location', label: 'location' },
    { key: 'Department', label: 'Department' },
    { key: 'Company name', label: 'Company Name' },
    { key: 'Employee Alias', label: 'Employee Alias' },
    { key: 'Change Date', label: 'Change Date' },
    { key: 'Joining Date', label: 'Joining Date' },
    { key: 'Employee Separation Date', label: 'notice period' },
    { key: 'Rep. Manager', label: 'Rep. Manager' },
    { key: 'HOD Names', label: 'HOD Names' },
    { key: 'HOD TMS Names', label: 'HOD TMS Names' },
    { key: 'Cost Center', label: 'Cost Center' },
    { key: 'Gender', label: 'Gender' },
    { key: 'Date Of Birth', label: 'Date Of Birth' },
    { key: 'Blood Group', label: 'Blood Group' },
    { key: 'Indicator', label: 'Indicator' },
    { key: 'Country/Region Key', label: 'Country/Region Key' },
    { key: 'Bank Account', label: 'Bank Account' },
    { key: 'Bank Country/Region', label: 'Bank Country/Region' },
    { key: 'Bank Number', label: 'Bank Number' },
    { key: 'Postal Code', label: 'Postal Code' },
    { key: 'Region', label: 'Region' },
    { key: 'Company Code', label: 'Company Code' },
    { key: 'IFSC Code', label: 'IFSC Code' },
    { key: 'Account holder', label: 'Account Holder' },
    { key: 'Nationality text', label: 'Nationality Text' },
    { key: 'State', label: 'State' },
    { key: 'Name of Financial Institution', label: 'Financial Institution' },
    { key: 'Date', label: 'Date' },
    { key: 'Character Field with Length 10', label: 'Field Length 10' },
    { key: 'Payroll area', label: 'Payroll Area' },
    { key: 'Flag on and off', label: 'Flag On/Off' },
    { key: 'Record Updated or not', label: 'Record Updated' },
    { key: 'Days Overdue', label: 'Days Overdue' },
    { key: 'Ghost Aging Bucket', label: 'Aging Bucket' }
];
...
const DataTable = ({ filters, onFilterChange, debouncedSearch }) => {
    ...
    useEffect(() => { setCurrentPage(1); }, [filters.employee_id, filters.department, filters.notice_bucket, filters.cost_center, filters.employee_status, filters.location, filters.employee, filters.startDate, filters.endDate, pageSize, debouncedSearch]);
    ...
    useEffect(() => {
        ...
        const doFetch = async () => {
            ...
            try {
                ...
                const res = await fetch(`http://localhost:5000/api/pjpa39/table?${qs}`);
    ...
    }, [filters.employee_id, filters.department, filters.notice_bucket, filters.cost_center, filters.employee_status, filters.location, filters.employee, filters.startDate, filters.endDate, currentPage, pageSize, debouncedSearch]);

    const handleDownloadCSV = useCallback(async () => {
        try {
            setCsvLoading(true);
            const qs = buildQuery(filters, { page: 1, page_size: 100000, search: debouncedSearch || undefined });
            const res = await fetch(`http://localhost:5000/api/pjpa39/table/csv?${qs}`);
            const text = await res.text();

            const blob = new Blob([text], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = 'ghost_employees_dashboard.csv'; a.click();
        ...
        <div className="data-table-card">
            <div className="table-header-row" style={{ alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <h3 className="chart-title" style={{ margin: 0 }}>Ghost Employees Data</h3>
