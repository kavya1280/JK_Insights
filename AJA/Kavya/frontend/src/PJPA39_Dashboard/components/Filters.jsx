...
const FilterIcons = {
    ...
    department: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5z" /><path d="M2 17l10 5 10-5M2 12l10 5 10-5" /></svg>),
    notice: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>),
    costCenter: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2" /><line x1="1" y1="10" x2="23" y2="10" /></svg>),
...
const handleScreenshot = async () => {
    try {
        ...
        link.download = `pjpa39_dashboard_${new Date().toISOString().slice(0, 10)}.png`;
...
const Filters = ({ options, filters, onFilterChange, onReset }) => {
    const uniqueValues = {
        employee_id: options?.employee_ids || [],
        employee: options?.employee_names || [],
        location: options?.locations || [],
        employee_status: options?.employee_statuses || []
    };
    ...
    const filterConfigs = [
        ...
        { name: 'employee', label: 'Employee', icon: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>), placeholder: 'All' },
        { name: 'location', label: 'Location', icon: (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>), placeholder: 'All' },
        { name: 'employee_status', label: 'Status', icon: FilterIcons.status, placeholder: 'All' }
    ];
        ...
        <div className="filters-section">
            ...
            <div className="filters-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginTop: '20px' }}>
                ...
                <div className="filter-group">
                    <label className="filter-label">
                        <span className="filter-label-icon" style={{ color: '#6FAE2C' }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg></span>Document Date
                    </label>
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        <input type="date" value={filters.startDate} onChange={e => handleChange('startDate', e.target.value)}
                            style={{ padding: '8px 10px', background: 'white', border: '1.5px solid #E5E7EB', borderRadius: '10px', fontSize: '13px', flex: 1, outline: 'none' }} />
