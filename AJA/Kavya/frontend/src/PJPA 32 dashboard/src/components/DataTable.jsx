...
const PREFERRED = [
    ...
    { key: 'approvedAmount', label: 'Approved Amt', isAmount: true },
    { key: 'totalApprovedAmount', label: 'Total Amt', isAmount: true },
    ...
    { key: 'year', label: 'Year' },
    { key: 'personBand', label: 'Person Band' },
];
...
const DataTable = ({ filters, onFilterChange, debouncedSearch }) => {
    ...
    // Local state for the search input before it's debounced in parent
    const [localSearch, setLocalSearch] = useState(filters.search || '');

    // Sync local search when filters are reset from parent
    useEffect(() => {
        setLocalSearch(filters.search || '');
    }, [filters.search]);

    // Update parent search when typing
    ...
    // Reset page on filter / search / size changes
    useEffect(() => { setCurrentPage(1); }, [filters.employeeId, filters.employee, filters.reportId, filters.expenseType, filters.location, pageSize, debouncedSearch]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    useEffect(() => {
        ...
        const doFetch = async () => {
            ...
            try {
                ...
                const res = await fetch(`http://localhost:5000/api/pjpa32/all-data?${qs}`);
    ...
    }, [filters.employeeId, filters.employee, filters.reportId, filters.expenseType, filters.location, currentPage, pageSize, debouncedSearch]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    const handleDownloadCSV = useCallback(async () => {
        try {
            ...
            const csvLines = [
                ...
                ...rows.map(row => cols.map(c => escape(formatVal(c, row[c.key]))).join(','))
            ];
            const blob = new Blob([csvLines.join('\r\n')], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = 'travel_claims.csv'; a.click();
    ...
    }, [filters.employeeId, filters.employee, filters.reportId, filters.expenseType, filters.location, columns, debouncedSearch]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <div className="data-table-card">
            ...
            <div className="table-header-row" style={{ alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <h3 className="chart-title" style={{ margin: 0 }}>Travel Claims Data</h3>
            ...
            {/* ── Search (Left) and Dropdown (Right) ── */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, gap: 15 }}>
                ...
                <div style={{
                    ...
                    {localSearch && (
                        <button
                            onClick={() => handleLocalSearchChange({ target: { value: '' } })}
                            style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#9E9E9E' }}
                        >✕</button>
                    )}
                </div>

                {/* Entry Dropdown (Top Right) */}
            ...
            {/* ── Table Area with Non-Interruptive Loading ── */}
            <div className="table-responsive" style={{ position: 'relative' }}>
                {/* Thin loading bar at top of table */}
                {loading && (
                    <div style={{
                        position: 'absolute', top: 0, left: 0, right: 0,
                        height: '3px', background: '#DCF0B2', overflow: 'hidden', zIndex: 10
                    }}>
                        <div className="loading-bar-animation" style={{
                            height: '100%', background: '#6FAE2C', width: '30%'
                        }} />
                    </div>
                )}
                        ...
                        <table className="enterprise-table">
                            <thead>
                                <tr>
                                    ...
                                    {columns.map(col => (
                                        <th key={col.key} className={col.isAmount ? 'text-right' : ''}>{col.label}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {data.length > 0 ? data.map((row, idx) => (
                                    ...
                                    <tr>
                                        <td colSpan={(columns.length || 1) + 1} className="td-empty">
                                            {debouncedSearch ? `No results for "${debouncedSearch}"` : 'No data available'}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
            ...
            {!loading && !error && (
                <div className="pagination-controls">
                    ...
                    <span className="pagination-info">
                        Showing <strong>{total > 0 ? startIndex + 1 : 0}&ndash;{Math.min(startIndex + pageSize, total)}</strong> of <strong>{total.toLocaleString()}</strong>
