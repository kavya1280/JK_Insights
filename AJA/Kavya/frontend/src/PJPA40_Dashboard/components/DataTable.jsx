...
const PREFERRED = [
    ...
    { key: 'Total Approved Amount', label: 'Total Approved Amount', isAmount: true },
    { key: 'Is Anomalous', label: 'Is Anomalous' },
...
const formatVal = (col, val) => {
    ...
    if (col.key === 'Is Anomalous' && (val === 'Yes' || val === 'True' || val === true)) {
        return <span style={{ color: '#EF5350', fontWeight: 'bold' }}>Yes</span>;
    }
    return String(val);
};
...
const DataTable = ({ filters, onFilterChange, debouncedSearch }) => {
    ...
    useEffect(() => { setCurrentPage(1); }, [filters.employee, filters.expense_type, filters.policy, pageSize, debouncedSearch]);
    ...
    useEffect(() => {
        ...
        const doFetch = async () => {
            ...
            try {
                ...
                const res = await fetch(`http://localhost:5000/api/pjpa40/table?${qs}`);
    ...
    }, [filters.employee, filters.expense_type, filters.policy, currentPage, pageSize, debouncedSearch]);

    const handleDownloadCSV = useCallback(async () => {
        try {
            setCsvLoading(true);
            const qs = buildQuery(filters, { page: 1, page_size: 100000, search: debouncedSearch || undefined });
            const res = await fetch(`http://localhost:5000/api/pjpa40/table/csv?${qs}`);
            const text = await res.text();

            const blob = new Blob([text], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = 'concur_nonconcur_dashboard.csv'; a.click();
        ...
        <div className="data-table-card">
            <div className="table-header-row" style={{ alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <h3 className="chart-title" style={{ margin: 0 }}>Compliance & Spend Data</h3>
