...
const TotalSpendByEmployeeChart = ({ data }) => {
    const chartData = useMemo(() => {
        ...
        data.forEach(item => {
            ...
            map.set(emp, (map.get(emp) || 0) + amt);
        });

        return Array.from(map.entries())
            .map(([emp, amount]) => ({ emp, amount }))
    ...
    return (
        <div className="chart-card chart-animate equal-height-card">
            <div className="chart-title-row">
                ...
                <h3 className="chart-title">Total Spend by Employee</h3>
            </div>
                ...
                {chartData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 60 }}>
                            ...
                            />
                            <Bar dataKey="amount" fill="#0B4F94" radius={[4, 4, 0, 0]} barSize={20} />
                        </BarChart>
                    </ResponsiveContainer>
                ) : (
...
export default TotalSpendByEmployeeChart;
