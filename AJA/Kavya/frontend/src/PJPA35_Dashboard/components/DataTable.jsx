...
const PREFERRED = [
    ...
    { key: 'Duplicate Count', label: 'Duplicate Count' },
    ...
    { key: 'Exception Reason', label: 'Exception Reason' },
...
const DataTable = ({ filters, onFilterChange, debouncedSearch }) => {
    ...
    useEffect(() => { setCurrentPage(1); }, [filters.employee_id, filters.employee, filters.report_id, pageSize, debouncedSearch]);
    ...
    useEffect(() => {
        ...
        const doFetch = async () => {
            ...
            try {
                ...
                const res = await fetch(`http://localhost:5000/api/pjpa35/table?${qs}`);
    ...
    }, [filters.employee_id, filters.employee, filters.report_id, currentPage, pageSize, debouncedSearch]);
    ...
    const handleDownloadCSV = useCallback(async () => {
        try {
            ...
            const res = await fetch(`http://localhost:5000/api/pjpa35/table/csv?${qs}`);
            const text = await res.text();

            const blob = new Blob([text], { type: 'text/csv;charset=utf-8;' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url; a.download = 'duplicate_report_id_dashboard.csv'; a.click();
        ...
        <div className="data-table-card">
            <div className="table-header-row" style={{ alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <h3 className="chart-title" style={{ margin: 0 }}>Duplicate Reports Data</h3>
